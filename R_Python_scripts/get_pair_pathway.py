...
reactome = pd.read_csv(reactome_file, sep="\t", header=None, 
                       usecols=[0, 3, 5], names=["ensembl_id", "pathway_name", "species"])

reactome = reactome[reactome["species"] == "Homo sapiens"]

immune = pd.read_csv(immune_file)
immune["gene_id"] = immune["gene_id"].str.strip()
reactome["ensembl_id"] = reactome["ensembl_id"].str.strip()

immune_ids = set(immune["gene_id"].unique())
reactome_immune = reactome[reactome["ensembl_id"].isin(immune_ids)]

pathways = reactome_immune[["pathway_name"]].drop_duplicates().reset_index(drop=True)

def categorize_pathway(name):
    n = str(name).lower()
    if any(x in n for x in ["immune system", "chemokine", "interleukin", "interferon", 
                             "cytokine", "adaptive immune", "innate immune",
...
pathways["pathway_id"] = range(1, len(pathways) + 1)
